...
class Customdataset(Dataset):
    ...
    def __getitem__(self, index):
        vis_path = os.path.join(self.rgb_dataset, self.image_rgb_paths[index])
        ir_path = os.path.join(self.ir_dataset, self.image_ir_paths[index])
        img1 = Image.open(vis_path)
        img2 = Image.open(ir_path)
