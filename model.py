...
from kornia.filters import Laplacian, SpatialGradient, GaussianBlur2d
...
class SEblock(nn.Module):
    def __init__(self, in_channels, reductions=4):
        super(SEblock, self).__init__()
        self.squeeze_avg = nn.AdaptiveAvgPool2d(1)
        self.excitation = nn.Sequential(
            nn.Linear(in_channels, in_channels//reductions, bias=False),
            nn.ReLU(inplace=True),
            
            nn.Linear(in_channels//reductions, in_channels, bias=False),
            nn.Sigmoid()
        )
